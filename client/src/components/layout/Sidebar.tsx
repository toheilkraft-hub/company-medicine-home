...
export default function Sidebar({ open, onClose }: SidebarProps) {
    ...
    return (
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <p className={cn("text-xs font-medium truncate", isActive ? "text-brand-700" : "text-gray-600")}>
            {truncate(conv.title, 36)}
          </p>
          <p className="text-xs text-gray-400">{timeAgo(conv.updatedAt)}</p>
        </div>
        <button
          className="opacity-0 group-hover:opacity-100 p-1 rounded text-gray-400 hover:text-gray-600 shrink-0 transition-all"
          onClick={(e) => {
            e.stopPropagation();
  ...
  return (
    <>
      ...
      <aside className={cn(
        "fixed lg:relative z-30 lg:z-auto flex flex-col w-64 h-full bg-white border-r border-gray-200 transition-transform duration-200",
        open ? "translate-x-0" : "-translate-x-full lg:translate-x-0"
      )}>
        {/* Logo */}
        <div className="flex items-center justify-between px-4 h-13 py-3 border-b border-gray-200 shrink-0">
          <div className="flex items-center gap-2.5">
            <div className="w-7 h-7 rounded-lg bg-brand-600 flex items-center justify-center">
              <Cpu size={15} className="text-white" />
            </div>
            <span className="font-bold text-gray-900 text-base tracking-tight">iHeal AI</span>
          </div>
        ...
        <div className="px-3 pb-2 shrink-0">
          <input
            ...
            className="w-full bg-gray-50 border border-gray-200 text-gray-700 text-xs rounded-lg px-3 py-1.5 placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-brand-500"
          />
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-2 space-y-0.5">
          {pinned.length > 0 && (
            <>
              <p className="text-xs text-gray-400 uppercase tracking-wider px-2 pt-2 pb-1 font-semibold">Pinned</p>
        ...
        {contextMenu && (
          <div
            ref={menuRef}
            className="fixed z-50 bg-white border border-gray-200 rounded-xl shadow-lg py-1 w-40"
        ...
        <div className="border-t border-gray-200 px-2 py-2 shrink-0">
          {navItems.map(({ icon: Icon, label, href }) => (
            <Link key={href} href={href}>
              <div className={cn(
                ...
                location === href ? "bg-gray-100 text-gray-800" : "text-gray-500 hover:bg-gray-50 hover:text-gray-700"
              )}>
                <Icon size={14} />
                {label}
              </div>
          ...
          <Link href="/profile">
            <div className="flex items-center gap-2 px-3 py-2 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors mt-1">
              <div className="w-5 h-5 rounded-full bg-brand-600 flex items-center justify-center text-xs font-bold text-white shrink-0">
                {user?.name?.[0]?.toUpperCase()}
              </div>
              <span className="text-xs text-gray-500 truncate flex-1">{user?.name}</span>
            </div>
          </Link>
        </div>
      </aside>
